...
import "../middlewares/passport.middlewares";
...
export class AuthController {
    ...
    public authDeviceJWT(req: Request, res: Response, next: NextFunction) {
        passport.authenticate("jwt", function (err, device, info) {
            if (err) {
                console.log(err);
                return res.status(401).json({ status: "error", code: "unauthorized" });
            }
